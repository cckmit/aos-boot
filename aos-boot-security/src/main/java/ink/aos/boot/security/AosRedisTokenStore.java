...
@Slf4j
public class AosRedisTokenStore implements TokenStore {
    ...
    private AuthenticationKeyGenerator authenticationKeyGenerator = new AosAuthenticationKeyGenerator();
    ...
    public AosRedisTokenStore(RedisConnectionFactory connectionFactory) {
    ...
    public void removeAccessToken(String tokenValue) {
        log.debug("removeAccessToken : {}", tokenValue);
...
//            conn.del(accessToRefreshKey);
            conn.expire(accessKey, 60);
            conn.expire(accessToRefreshKey, 60);
            // Don't remove the refresh token - it's up to the caller to do that
//            conn.del(authKey);
            conn.expire(authKey, 60);
...
//                conn.del(authToAccessKey);
                conn.expire(authToAccessKey, 60);
                conn.sRem(unameKey, access);
                conn.sRem(clientId, access);
//                conn.del(serialize(ACCESS + key));
                conn.expire(serialize(ACCESS + key), 60);
                conn.closePipeline();
            }
        } catch (Exception e) {
    ...
    public void removeRefreshToken(String tokenValue) {
        log.debug("removeRefreshToken : {}", tokenValue);
...
//            conn.del(access2RefreshKey);
            conn.expire(refreshKey, 60);
            conn.expire(refreshAuthKey, 60);
            conn.expire(refresh2AccessKey, 60);
            conn.expire(access2RefreshKey, 60);
            conn.closePipeline();
        } finally {
            conn.close();
        }
    ...
    private void removeAccessTokenUsingRefreshToken(String refreshToken) {
        log.debug("removeAccessToken : {}", refreshToken);
