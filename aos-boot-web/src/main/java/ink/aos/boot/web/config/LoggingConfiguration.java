...
@EnableConfigurationProperties(AosLogProperties.class)
public class LoggingConfiguration {
    ...
    private final AosLogProperties aosLogProperties;

    public LoggingConfiguration(@Value("${spring.application.name}") String appName,
                                Optional<BuildProperties> buildProperties, AosLogProperties aosLogProperties) {
        this.appName = appName;
        this.version = buildProperties.isPresent() ? buildProperties.get().getVersion() : "";
        this.aosLogProperties = aosLogProperties;
        if (this.aosLogProperties.isUseJsonFormat()) {
            addJsonConsoleAppender(context);
        }
        if (this.aosLogProperties.getLogstash().isEnabled()) {
            addLogstashTcpSocketAppender(context);
        }
        if (this.aosLogProperties.isUseJsonFormat() || this.aosLogProperties.getLogstash().isEnabled()) {
            addContextListener(context);
        }
//        if (this.jHipsterProperties.getMetrics().getLogs().isEnabled()) {
//            setMetricsMarkerLogbackFilter(context);
//        }
        if (!aosLogProperties.getFile().isEnabled()) {
            context.getLogger(ch.qos.logback.classic.Logger.ROOT_LOGGER_NAME).detachAppender("FILE");
        }
    }
    ...
    private void addLogstashTcpSocketAppender(LoggerContext context) {
        ...
        logstashAppender.addDestinations(new InetSocketAddress(this.aosLogProperties.getLogstash().getHost(), this.aosLogProperties.getLogstash().getPort()));
        ...
        asyncLogstashAppender.setQueueSize(this.aosLogProperties.getLogstash().getQueueSize());
...
//        customFields.append(",").append("\"app_port\":\"").append(this.serverPort).append("\"");
    ...
    private void addContextListener(LoggerContext context) {
        LogbackLoggerContextListener loggerContextListener = new LogbackLoggerContextListener(aosLogProperties);
        ...
        context.getLoggerList().forEach((logger) -> {
            for (Iterator<Appender<ILoggingEvent>> it = logger.iteratorForAppenders(); it.hasNext(); ) {
                ...
                if (!appender.getName().equals(ASYNC_LOGSTASH_APPENDER_NAME)
                        && !(appender.getName().equals(CONSOLE_APPENDER_NAME) && this.aosLogProperties.isUseJsonFormat())) {
    ...
    class LogbackLoggerContextListener extends ContextAwareBase implements LoggerContextListener {
        ...
        private LogbackLoggerContextListener(AosLogProperties aosLogProperties) {
            this.aosLogProperties = aosLogProperties;
        }
